...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    int x = point.x;
    int y = point.y;
    NSString * xy = [NSString stringWithFormat:@"%d,%d",x,y];
    NSLog(@"touch123)");
    NSLog(@"touch(%d,%d)",x,y);
    //if (x>1244 && x<1604 && y>1116 && y<1240) {
    
    NSDictionary * dict = [[NSDictionary alloc] initWithObjectsAndKeys:xy, @"point", nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:TOUCH_SCREEN_NOTIFICATION  object:self userInfo:dict];
    NSLog(@"123");
    // }
